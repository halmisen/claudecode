# Doji Ashi Strategy V5 - 使用指南和注意事�?
...
# 可选依�?(推荐)
pip install TA-Lib  # 技术指标计算加�?
```

### 2. 数据要求
- **格式**: OHLCV CSV文件，包�?`open_time, open, high, low, close, volume` �?
- **时间格式**: Unix时间戳（毫秒）或标准datetime格式
- **数据来源**: 支持Binance、其他交易所的标准格�?
- **建议时间框架**: 2小时�?小时（经过优化测试）

### 3. 文件路径
- **主策�?*: `backtester/strategies/doji_ashi_strategy_v5.py`
...
- **仓位大小**: 默认20%单笔仓位�?倍杠�?
- **最大回�?*: 监控不超�?0%的回�?
- **交易方向**: 推荐只做多（long），风险更可�?

### 3. 市场过滤�?
- **加密货币模式**: 自动使用BTC作为市场过滤�?
- **股票模式**: 使用SPY作为市场过滤�?
...
### 从V4迁移到V5
1. **备份V4环境**: 保留V4文件作为参�?
2. **安装新依�?*: `pip install backtrader-plotting`
3. **移除旧依�?*: `pip uninstall plotly plotly-resampler`
4. **更新运行命令**: 使用V5的新参数格式
5. **验证结果**: 对比V4和V5的回测结�?
...
### 3. 开发支�?
- 查看 `docs/development_log_v5_final_solution.md` 了解技术细�?
- 参�?`CLAUDE.md` 获取完整的项目文�?
- 使用 `docs/project-planning/GEMINI.md` 作为第三方AI助手的指�?
