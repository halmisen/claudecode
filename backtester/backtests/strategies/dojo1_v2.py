...
Baseline Derived From: backtester/backtests/strategies/dojo1.py
...
class DojiAshiStrategyV2(bt.Strategy):
    params = (
        ...
        ("order_percent", 0.10),  # fraction of equity per trade (safer default)
        ("min_size", 0.0001),
        ("cooldown_bars", 10),
        ("size_step", 0.0),              # optional exchange step size; 0 to disable
        ("leverage", 1.0),               # sizing-side leverage factor; keep 1.0 if broker already applies leverage
        ("maker_mode", True),            # place limit orders instead of market to reduce slippage
        ("maker_limit_offset_percent", 0.0),  # optional tiny offset for limit price
        ("pending_order_timeout_bars", 10),   # max bars to wait for parent fill before cancel
        ("warmup_daily", 200),           # minimum daily bars required before signals
    ...
    def __init__(self):
        ...
        self.parent_submit_bar_index = None
        self.last_long_bar = -10 ** 9
        self.last_short_bar = -10 ** 9

    # -------- Utility helpers --------
    def _gc_orders(self):
        # Garbage-collect any completed/rejected orders to prevent stale references
        for name in ("parent_order", "sl_order", "tp_order", "trail_order"):
            order_ref = getattr(self, name)
            try:
                if order_ref is not None and not order_ref.alive():
                    setattr(self, name, None)
            except Exception:
                # Be defensive if order_ref does not expose alive()
                pass

    def _confirmed_daily(self, line):
        # 使用传入 line 的自身长度来判断是否有“已确认”的上一根日K
        # 当不足 2 根（仅有当前未收盘的 1 根）时，返回 False 以跳过信号
        try:
            if len(line) >= 2:
                return line[-1]
        except Exception:
    ...
    def _calc_size(self) -> float:
        ...
        if equity <= 0 or self.data_close[0] <= 0:
            return 0.0
        position_value = equity * float(self.p.order_percent) * float(self.p.leverage)
        raw_size = position_value / float(self.data_close[0])
        size = float(raw_size)
        step = float(self.p.size_step or 0.0)
        if step > 0.0:
            # round down to step
            size = (int(size / step)) * step
        return max(float(self.p.min_size), float(size))

    def _check_pending_order_timeout(self):
        # Cancel parent if it has been pending for too many bars (maker limit not filled)
        if not self.parent_order:
            return
        if self.parent_submit_bar_index is None:
            return
        try:
            waited = len(self) - int(self.parent_submit_bar_index)
            if waited >= int(self.p.pending_order_timeout_bars):
                try:
            ...
            # Be defensive: never let timeout logic crash next()
            pass

    # -------- Core Backtrader callbacks --------
    def next(self):
        # GC stale orders first
        self._gc_orders()

        # Warmup: need confirmed daily SMAs and trigger MAs
        min_warmup = max(int(self.p.warmup_daily), int(self.p.atr_length))
        if len(self.daily_data) < min_warmup:
            return
        ...
        if self.parent_order or self.sl_order or self.tp_order or self.trail_order:
            # For maker mode, enforce pending parent timeout
            self._check_pending_order_timeout()
        ...
        if allow_long:
            ...
            if size > 0:
                if bool(self.p.maker_mode):
                    # place limit buy at current close minus optional offset
                    offset = float(self.p.maker_limit_offset_percent or 0.0) / 100.0
                    price = float(self.data_close[0]) * (1.0 - offset)
                    self.parent_order = self.buy(size=size, exectype=bt.Order.Limit, price=price)
                    self.parent_submit_bar_index = current_bar
                else:
            ...
            if size > 0:
                if bool(self.p.maker_mode):
                    # place limit sell at current close plus optional offset
                    offset = float(self.p.maker_limit_offset_percent or 0.0) / 100.0
                    price = float(self.data_close[0]) * (1.0 + offset)
                    self.parent_order = self.sell(size=size, exectype=bt.Order.Limit, price=price)
                    self.parent_submit_bar_index = current_bar
                else:
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order == self.parent_order:
                ...
                # 子单（SL/TP/Trailing）完成后需清理引用，避免 next() 的早退“卡住”
                if order == self.sl_order:
                    ...
                    if self.tp_order is not None:
                        try:
                            ...
                            pass
                        self.tp_order = None
                    if self.trail_order is not None:
                        try:
                            self.cancel(self.trail_order)
                        except Exception:
                            pass
    ...
    def notify_trade(self, trade):
        # 兜底：当交易关闭时，确保全部引用清理，防止后续 next() 被阻塞
        if trade.isclosed:
            self.parent_order = None
            self.sl_order = None
            self.tp_order = None
...
if __name__ == '__main__':
    ...
    # --- Argument Parser ---
    parser = argparse.ArgumentParser(description='Run Doji Ashi Strategy V2 Backtest with multiple data feeds')
    parser.add_argument(
        '--main_data',
        default='D:/BIGBOSS/backtester/backtests/data/BTCUSDT/BTCUSDT_4h_merged.csv',
        help='Path to the main CSV data file (e.g., 4H BTCUSDT.)'
    )
    parser.add_argument(
        '--daily_data',
        default='D:/BIGBOSS/backtester/backtests/data/BTCUSDT/BTCUSDT_1d_merged.csv',
        help='Path to the daily CSV data file for trend filtering.'
    )
    args = parser.parse_args()

    # Preprocess main data
    main_data_preprocessed_path = os.path.join(os.path.dirname(args.main_data), "preprocessed_" + os.path.basename(args.main_data))
    subprocess.run(["D:\\BIGBOSS\\backtester\\venv\\Scripts\\python.exe", "D:\\BIGBOSS\\preprocess_data.py", args.main_data, main_data_preprocessed_path], check=True)

    # Preprocess daily data
    daily_data_preprocessed_path = os.path.join(os.path.dirname(args.daily_data), "preprocessed_" + os.path.basename(args.daily_data))
    subprocess.run(["D:\\BIGBOSS\\backtester\\venv\\Scripts\\python.exe", "D:\\BIGBOSS\\preprocess_data.py", args.daily_data, daily_data_preprocessed_path], check=True)

    cerebro = bt.Cerebro()

    # Add main data feed (4H)
    data_main = bt.feeds.GenericCSVData(
        dataname=main_data_preprocessed_path,
        fromdate=datetime.datetime(2020, 1, 1),
        todate=datetime.datetime(2023, 12, 31),
        timeframe=bt.TimeFrame.Minutes,
        compression=240,  # 4-hour data (240 minutes)
    ...
    # Add daily data feed (1D) for filtering
    data_daily = bt.feeds.GenericCSVData(
        dataname=daily_data_preprocessed_path,
        fromdate=datetime.datetime(2020, 1, 1),
        todate=datetime.datetime(2023, 12, 31),
        timeframe=bt.TimeFrame.Days,
        compression=1,  # 1-day data
        dtformat=('%Y-%m-%d %H:%M:%S'),
        datetime=0,
        open=1,
        high=2,
    ...
    # Add strategy
    cerebro.addstrategy(DojiAshiStrategyV2)

    # Set initial capital and broker commission
    ...
    # Plot the results with Bokeh if no plotter is specified
    # This is the patch: it automatically uses Bokeh if cerebro.plot() is called
    # without a 'plotter' argument.
    try:
        plotter = cerebro.plotter
    except AttributeError:
        # If plotter is not set, default to Bokeh
        plotter = None
